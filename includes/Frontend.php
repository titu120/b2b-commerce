...
class Frontend {
    ...
    public function order_history_shortcode() {
        ...
        if (empty($orders)) {
            ...
            echo '<div class="b2b-empty-icon">' . __('📦', 'b2b-commerce') . '</div>';
            echo '<h4>' . __('No Orders Yet', 'b2b-commerce') . '</h4>';
            echo '<p>' . __('You haven\'t placed any orders yet. Start shopping to see your order history here.', 'b2b-commerce') . '</p>';
            echo '<a href="' . esc_url(wc_get_page_permalink('shop')) . '" class="b2b-empty-action">' . __('Start Shopping', 'b2b-commerce') . '</a>';
            echo '</div>';
        } else {
            echo '<div class="b2b-order-table-wrapper">';
            echo '<table class="b2b-order-history-modern">';
            echo '<thead><tr>';
            echo '<th>' . __('Order', 'b2b-commerce') . '</th>';
            echo '<th>' . __('Date', 'b2b-commerce') . '</th>';
            echo '<th>' . __('Status', 'b2b-commerce') . '</th>';
            echo '<th>' . __('Total', 'b2b-commerce') . '</th>';
            echo '<th>' . __('Actions', 'b2b-commerce') . '</th>';
            echo '</tr></thead><tbody>';
            ...
            foreach ( $orders as $order ) {
                ...
                echo '<td><a href="' . esc_url( add_query_arg( [ 'b2b_invoice' => $order->get_id() ] ) ) . '" target="_blank" class="b2b-invoice-link">' . __('📄', 'b2b-commerce') . ' ' . __('Invoice', 'b2b-commerce') . '</a></td>';
                echo '</tr>';
            }
            echo '</tbody></table>';
            echo '</div>';
